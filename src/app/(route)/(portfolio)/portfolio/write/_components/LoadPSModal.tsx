  ...
  return (
    <Overlay onClick={handleOverlayClick}>
      <motion.div
        ...
        {...modalMotion}
        className="flex m-4 min-w-[360px] max-w-[1000px] max-h-[600px] hide-scrollbar overflow-y-scroll w-full px-4 sm:px-[65px] py-[58px] flex-col items-start gap-6 rounded-2xl bg-white-0 shadow-md"
      >
          ...
          psList?.map((ps: PSListBoxProps) => (
            <section className="flex justify-between w-full max-h-[400px] px px-6 pb-4 pt-6 sm:px-8 sm:pb-4 sm:pt-10 border border-gray-2 rounded-lg bg-white-0 cursor-pointer">
              <div>
                ...
                <p className="text-gray-0 text-xs sm:text-sm pt-4">{formatDate(ps.timestamp)} 등록</p>
              </div>
              <button
                type="button"
                onClick={() => {
